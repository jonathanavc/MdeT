...
for thead in theads:
    for i in range(1, num_ex):
        ...
        p = subprocess.Popen(['time', './omp_ex', str(thead)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
for bloq in cuda_bloqs:
    for thead in cuda_threads: 
        for i in range(1, num_ex):
            ...
            p = subprocess.Popen(['time','./cuda2_ex', str(bloq), str(theads)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = p.communicate()
