...
__device__ double cal_tnf_dist(size_t r1, size_t r2, double *TNF, size_t *seqs_d_index, size_t seqs_d_index_size) {
    ...
    b = 46349.1624324381;
    if (b != b) return -1;
    b += -76092.3748553155 * lw11;
    if (b != b) return -2;
    b += -639.918334183 * lw21;
    if (b != b) return -3;
    b += 53873.3933743949 * lw12;
    if (b != b) return -4;
    b += -156.6547554844 * lw22;
    if (b != b) return -5;
    b += -21263.6010657275 * lw13;
    if (b != b) return -6;
    b += 64.7719132839 * lw23;
    b += 5003.2646455284 * lw14;
    b += -8.5014386744 * lw24;
    b += -700.5825500292 * lw15;
    b += 0.3968284526 * lw25;
    if (b != b) return -7;
    b += 54.037542743 * lw16;
    b += -1.7713972342 * lw17;
    b += 474.0850141891 * lw11 * lw21;
    b += -23.966597785 * lw12 * lw22;
    if (b != b) return -8;
    b += 0.7800219061 * lw13 * lw23;
    b += -0.0138723693 * lw14 * lw24;
    b += 0.0001027543 * lw15 * lw25;
    if (b != b) return -9;
