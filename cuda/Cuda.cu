...
__device__ double cal_tnf_dist_d(size_t r1, size_t r2, float *__restrict__ TNF1, float *__restrict__ TNF2) {
    ...
    d = sqrt(d);
    double b, c;
    double ctg1 = min((double)r1, (size_t)500000);
    double ctg2 = min((double)r2, (size_t)500000);
    double lw[19];
...
__global__ void get_tnf_prob(double *__restrict__ tnf_dist, float *__restrict__ TNF, size_t *__restrict__ seqs_d_size, size_t _des,
    ...
    while (tnf_dist_index != _limit2) {
        ...
        while (r2 < r1) {
            ...
            tnf_dist[tnf_dist_index] = cal_tnf_dist_d(seqs_d_size[r1], seqs_d_size[r2], _TNF, TNF_r2);
            tnf_dist_index++;
            TNF_r2 += 136;
            r2++;
        }
