...
template <class T, class C> class ranker {
private:
  const T *p;
  uint sz;
...
public:
  ...
  template <class S> void get_orders(vector<S> &w) const {
    w.resize(sz);
    w.front() = 0;
  ...
  template <class S> void get_partial_orders(vector<S> &w, uint num) const {
    if (num > sz)
      num = sz;
    w.resize(sz);
    w.front() = 0;
  ...
  template <class S> void get_ranks(vector<S> &w, const string &method) const {
    w.resize(sz);
  ...
  void get_partial_ranks(vector<S> &w, const string &method, size_t num) const {
    ...
    if (method == "average") {
      for (uint c = 0, reps; c < num; c += reps) {
        reps = 1;
        while (c + reps < num && p[tmp[c]] == p[tmp[c + reps]])
          ++reps;
        for (uint k = 0; k < reps; ++k)
          w[tmp[c + k]] = S(2 * c + reps - 1) / 2 + 1;
      }
...
template <class T> inline T quantile(const vector<T> &v, const double q) {
  return quantile(&v[0], v.size(), q);
}

#endif
// see
// https://sites.google.com/site/jivsoft/Home/compute-ranks-of-elements-in-a-c---array-or-vector
