...
__global__ void get_tnf_graph(double* graph, float* TNF, double* contig_log, size_t nc1, size_t nc2, size_t off1, size_t off2) {
    ...
    size_t ct2 = off2 + r2;
    if (ct1 == ct2) return;
    graph[prob_index] = 1. - cal_tnf_dist_d2(contig_log[ct1], contig_log[ct2], TNF + ct1 * 136, TNF + ct2 * 136);
}
...
__global__ void get_tnf_max_prob_sample(double* max_dist, float* TNF, double* size_log, size_t* contigs, size_t nobs, size_t _des,
    ...
    while (contig_idx != limit) {
        ...
        for (size_t i = 0; i < nobs; i++) {
            ...
            double dist = 1. - cal_tnf_dist_d2(size_log[contigs[contig_idx]], size_log[contigs[i]], TNF1, TNF + contigs[i] * 136);
            if (dist > local_max) {
                local_max = dist;
            }
        }
