    ...
    while (tnf_dist_index != _limit2) {
        ...
        while (r2 < nobs) {
            ...
            tnf_dist[tnf_dist_index] =
                cal_tnf_dist_d2(size_log[contigs[r1]], size_log[contigs[r2]], TNF + contigs[r1] * 136, TNF + contigs[r2] * 136);
...
#pragma omp parallel for
    for (size_t j = 0; j < nobs; ++j) {
        for (size_t i = 0; i < _nobs; ++i) {
            ...
            if (matrix_h[i * nobs + j] != s) {
                cerr << "Error: matrix_h[i * nobs + j] != s" << endl;
                exit(1);
            }
            matrix[i * nobs + j] = s;
        }
