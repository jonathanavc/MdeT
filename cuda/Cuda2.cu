...
#pragma omp declare reduction(merge_map_sv_st : std::unordered_map<std::string_view, size_t> : omp_out.merge(omp_in))
                size_t char_per_thread = (fsize + numThreads - 1) / numThreads;
#omp parallel for reduction(+ : nobs, nobs1) reduction(merge_string_view : contig_names, seqs, small_contig_names, small_seqs) reduction(merge_map_sv_st: contigs, small_contigs) shared(os)
