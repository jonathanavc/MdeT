...
__device__ double cal_tnf_pre_dist_d2(double r1, double r2, float* TNF1, float* TNF2) {
    double d = 0;
    float diff = 0;
...
void launch_tnf_max_prob_sample_kernel_multi(std::vector<size_t> idx, double* max_dist_h, size_t _nobs) {
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        cudaMemcpy(max_dist_h + i * contigs_per_device, max_dist_d[i], contigs_device * sizeof(double), cudaMemcpyDeviceToHost);
